#' @title Signed positions of points projections to spline curves (OP method)
#' @description This function defines a sign according to the parity of turns and the dot product of
#' footpoint projections with the osculating plane at the footpoint, described in the PhD thesis as the osculating plane method (OP).
#' The procedure is identical as described in \code{\link{bspline_parity_turns}}. 
#' @param c_points The current control points, expressed as a matrix of row vectors.
#' @param points The projected points, as a matrix of row vectors.
#' @param fp.u The parameter vector of the footpoints. If \code{NULL} the algorithm would try to 
#' generate them from \link{bspline_footpoint} with parameters \code{fp.*} (see below). 
...
#' @param max.iterations The maximum number of iterations allowed, above which the routine
#' ends  for each points. The condition is applied to each tested point independently.
#' @param initial.scan.points Number of initial points that are scanned when \code{fp.u} is not \code{NULL}.
#' @param knots The knots sequence of the spline. 
...
#' @param fp.tolerance, \code{fp.max.iterations}, \code{fp.initial.scan.points}, \code{fp.algorithm}, 
#' \code{fp.verbose}. These are parameters passed to \link{bspline_footpoint} in order to compute the footpoint
#' parameter value whenever it is set to \code{NULL}.
#' @param sign.op If 'normal' the full sign operation is found. 'parity' shows only the boolean over
#' the turn parity only. 'other' represents the other component in the operation, generally
#' a differential geometry operation. In this case the other component is defined by the sign of 
#' the normal vector on the osculating plane.
...
#' @example examples/bspline_parity_sign_example1.R
#' @author Máximo Sánchez-Aragón
#' @export
bspline_parity_sign <- function(  
                                c_points,
                                points,
                                fp.u = NULL,
                                degree = 3,
  ...
  pd_x_pdd <- (get.modulus.M(pd_x_pdd)^-1) * pd_x_pdd 
  fd  <- get.dotp.M(pd_x_pdd, projv)
